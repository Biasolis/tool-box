<template>
  <div class="tool-card">
    <div class="card-header">
      <h3>PNG para PDF</h3>
      <p>Converta uma ou mais imagens PNG em um único arquivo PDF.</p>
    </div>

    <div class="card-content">
      <div v-if="files.length === 0" class="file-input-area" @click="triggerFileInput">
        <input type="file" ref="fileInput" @change="handleFileSelect" multiple accept="image/png" style="display: none;" />
...
const convertToPdf = async () => {
  ...
  try {
    ...
    const response = await axios.post('/api/pdf-tools/png-to-pdf', formData, {
...
.action-btn.primary { background-color: #6f42c1; color: white; border-color: #6f42c1; } /* Cor Roxo/Azulado */
.action-btn.primary:hover { background-color: #5a32a3; border-color: #5a32a3; }
.action-btn.primary:disabled { background-color: #b19cd9; border-color: #b19cd9; cursor: not-allowed; }
